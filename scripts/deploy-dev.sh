...
################################################################################
echo ""
echo "[1/5] Bootstrapping AWS CDK..."
cd "$CDK_DIR"
npm install
npx cdk bootstrap "aws://${CDK_DEFAULT_ACCOUNT:-$(aws sts get-caller-identity --query Account --output text)}/$REGION"
echo "  ✓ CDK bootstrapped"

################################################################################
# Step 2: Deploy CDK stack (VPC, EKS, ECR, namespaces)
################################################################################
echo ""
echo "[2/5] Deploying WorkshopPlatformDev stack (VPC, EKS, ECR, namespaces)..."
...
echo "Stack outputs:"
cd "$CDK_DIR"
npx cdk context --json 2>/dev/null || echo "  (run 'npx cdk outputs WorkshopPlatformDev' manually)"
echo ""
